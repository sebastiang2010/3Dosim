function f_genero_fuente_3(archivo,tvoxel,cell,I,A,fuentes) 
...
h=waitbar(0,'Generando la Posicion de la fuente...');
n_total=length(x);
for i=1:n_total
    waitbar(i/n_total);
    if linea==1 % escritura de la primera linea
        if n==1 %primera fuente
            fprintf(fid,'  (%g',I(ind(i))); % colocar el universo
        else % segunda fuente
            fprintf(fid,'  (%g',I(ind(i))); %colocar el universo
        end
    end
    
    if linea==2 %escritura del resto
        if n==1
            fprintf(fid,'       (%g',I(ind(i))); % primera fuente
        else
            fprintf(fid,'  (%g',I(ind(i))); %segunda fuente
        end
    end
    ...
    if n==2 %se escriben dos fuentes por linea
        fprintf(fid,'\n');
        linea=2;
        n=1;
    else
...
close(h)
if mod(n_total,2)~=0;fprintf(fid,' \n');end
...
ind=find(A(:,:,:)>0);
% A=A./sum(A(:)); 
%nv_fuente=nv_fuente+length(i);
h=waitbar(0,'Generando las probalidades de la fuente...');
for i=1:n_total
    waitbar(i/n_total);
    fprintf(fid,'   %e',A(ind(i)));
    n1=n1+1;
    if n1>=5
        fprintf(fid,'\n');
        fprintf(fid,'    ');
...
if mod(n_total,4)~=0;fprintf(fid,' \n');end
