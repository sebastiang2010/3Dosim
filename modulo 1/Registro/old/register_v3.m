...
R_CT2 = imref2d(size(a), [ippCT(1), ippCT(1) + vCT(1) * (size(CT,2) - 1)], ...
                      [ippCT(2), ippCT(2) + vCT(2) * (size(CT,1) - 1)]);
R_PET2 = imref2d(size(b), [ippPET(1), ippPET(1) + vPET(1) * (size(PET,2) - 1)], ...
                       [ippPET(2), ippPET(2) + vPET(2) * (size(PET,1) - 1)]);

figure(100)
imshow(a./max(a(:)),R_CT2)
figure(101)
h=imshow(PET(:,:,50),R_PET2); 
%set(h, 'AlphaData', 0.5);
...
% Crear un segundo conjunto de ejes para la imagen PET y superponerlo
ax2 = axes;
imshow(b ./ max(b(:)), R_PET2, 'Parent', ax2);  % Mostrar PET en su sistema de referencia
set(ax2, 'Color', 'none');  % Hacer transparente el fondo de ax2
set(ax2.Children, 'AlphaData', 0.5);  % Ajustar transparencia de la imagen PET
colormap(ax2, jet);
colorbar(ax2);

% Sincronizar los límites de los ejes para ambos conjuntos
linkaxes([ax1, ax2]);

% Ocultar los ejes superiores (opcional)
