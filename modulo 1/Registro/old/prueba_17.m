...
% y=sPET(2)-y; 
 ...
 %pos2=[];
 %pos2(:,1) = R_PET.XWorldLimits(1) + (x - 1) * vPET(1);
 %pos2(:,2) = R_PET.YWorldLimits(1) + (y - 1) * vPET(2);
 %pos2(:,3) = R_PET.ZWorldLimits(1) + (z - 1) * vPET(3);

 d1=pos(:,2)-R_PET.YWorldLimits(1); 
 d2=pos(:,2)-R_PET.YWorldLimits(2); 
 pos3(:,2)=pos1(:,2)-(R_PET.YWorldLimits(2)-R_PET.YWorldLimits(1)); 
...
% z=1
% pos1=[]
% pos1(:,1) = R_PET.XWorldLimits(1) + (x - 1) * vPET(1);
% pos1(:,2) = R_PET.YWorldLimits(1) + (y - 1) * vPET(2);
% pos1(:,3) = R_PET.ZWorldLimits(1) + (z - 1) * vPET(3);
