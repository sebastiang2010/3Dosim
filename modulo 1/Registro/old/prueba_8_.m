...
% % ax2 = axes;
% % h = imshow(b ./ max(b(:)), R_PET2_moved, 'Parent', ax2);  % Mostrar PET en su sistema de referencia
% % h = imshow(b, R_PET2_moved, 'Parent', ax2);  % Mostrar PET en su sistema de referencia
% % set(ax2, 'Color', 'none');  % Hacer transparente el fondo de ax2
% % set(h, 'AlphaData', 0.5);  % Ajustar transparencia de la imagen PET
% % colormap(ax2, jet);  % Cambiar el mapa de colores solo en ax2
% % 5cb = colorbar(ax2, 'Position', [0.92, 0.1, 0.02, 0.8]); % Asociar el colorbar a ax2 y ajustar su posición
% % 
% % Opcional: Etiquetas para el colorbar
% % ylabel(5cb, 'Intensidad relativa PET', 'FontSize', 12);
% % 
% % axis(ax1, 'off');   % Ocultar ejes de CT
% % axis(ax2, 'off');   % Ocultar ejes de PET

%%
figure(510)
ax1 = axes;
imshow(a1 ./ max(a1(:)), R_CT2, 'Parent', ax1);  % CT normalizado
hold on
ax2 = axes;
% Mostrar PET con valores originales
h = imshow(b, R_PET2_moved, 'Parent', ax2);  
set(ax2, 'Color', 'none');  % Hacer transparente el fondo de ax2
set(h, 'AlphaData', 0.5);  % Ajustar transparencia de la imagen PET
colormap(ax2, jet);  % Cambiar el mapa de colores solo en ax2
%cb = colorbar(ax2, 'Position', [0.92, 0.1, 0.02, 0.8]);  % Asociar el colorbar a ax2 y ajustar su posición
clim(ax2, [min_value max_value]);  % Define manualmente los límites del colorbar
% Etiqueta descriptiva del colorbar
%ylabel(cb, 'Intensidad PET (cuentas o actividad)', 'FontSize', 12);
axis(ax1, 'on');   % Ocultar ejes de CT
axis(ax2, 'on');   % Ocultar ejes de PET
%linkaxes([ax1, ax2], 'xy'); % Desvincula los ejes
