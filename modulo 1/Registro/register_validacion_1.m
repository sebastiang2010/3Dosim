...
CT=paciente.CT_check;
R_CT=imref3d(size(CT)); 
...
% si esta en Bq 
% tengo que multiplicar por vCT./vPET
...
%if strcmp(UnitsPET,'Bq')
%   PET_interpolado=PET_interpolado./(prod(vPET)/prod(voxelSize_PET_interpolado)); 
%end 
...
% 
% % Después de crear R_PET_expandido:
% voxelSize_PET_expandido = [R_PET_expandido.PixelExtentInWorldX, ...
%                            R_PET_expandido.PixelExtentInWorldY, ...
%                            R_PET_expandido.PixelExtentInWorldZ];
% 
...
%     set(h,'Position',pos_bar)
%     clim([0 max6*escala])
%     title([' Expandido n slice : ', num2str(i)])
%     pause(0.1)
% end 
