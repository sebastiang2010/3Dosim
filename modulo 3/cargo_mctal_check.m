...
PET_org=double(PET_org);
Actividad=sum(PET_org(:));
A_GBq=Actividad/1e9;
...
%cell(end+1)=index.pretumor;
densidad(end+1)=densidad(2); %higado
%%
...
% for i=4:numel(cell) %solo para higado sano y tumor
%     organo=cell(i);
%     %if organo>=tumor
% 
...
nfig=nfig+1;
 ...
 %% grafico dosis fantoma 
            
            ax1=app.axes_CT_micro_2; 
            ax2=app.axes_D_micro_2;
           
            D1=app.tmp.Dosis.D2;
            Da=D1.hepatic_arteria;
            a=double(app.CT(:,:,slice));
            b=Da(:,:,slice);
            alphaMask = zeros(size(b));      
            alphaMask(b > 0) = app.tmp.transparecia;    
            ...
            set(ax2, 'CLim', [0 max2]);
            h_cb = colorbar(ax2);
            pos_bar = h_cb.Position;
            pos_bar(1) = pos_bar(1) + 0.01;
            set(h_cb, 'Position', pos_bar);
            h_cb.Label.String='Gy';
            h_cb.Label.Rotation = 90;
            h_cb.Label.FontWeight = 'bold';
            h_cb.Tag = 'colorbar_micro';
            set(h, 'HitTest', 'on');
            set(h, 'ButtonDownFcn', @(src, event) app.axesClickCallback(src, event, app.axes_D_micro_2));
            app.tmp.D2=b;
