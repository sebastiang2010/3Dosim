...
%vPET_o=paciente.vPET_org; 
% PET_original=paciente.PET_intp; 
...
%% version con R_SAND
% hay que considerar la cantida de micro esferas 
Dmicro(1).D=D.*R_SAND(1,1);                
Dmicro(2).D=D.*R_SAND(1,2);                 
Dmicro(3).D=D.*R_SAND(1,3);                  
Dmicro(4).D=D.*R_SAND(1,4);
R_SAND(1,5)=0.8; 
Dmicro(5).D=D.*R_SAND(1,5); 
